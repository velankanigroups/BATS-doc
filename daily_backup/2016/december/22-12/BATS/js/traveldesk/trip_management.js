batstravelDeskHome.controller('tripManagement', function($scope, $localStorage,$http,travelDeskFactory,travelDeskService) {
	...
	$scope.createTrip = function(){
		...
		if($scope.createTripData.apprx_start_time = ""){
			$scope.stime = false;
		}
		else{
			$scope.stime =true;
	...
	$scope.initTriplistMap = function(tripdetail){
		directionsService = new google.maps.DirectionsService();
		console.log(document.getElementById('triplist_map'+tripdetail.trip_id));
		...
		resizeTriplistMap(triplist_map);
	}
	function resizeTriplistMap(triplist_map) {
		console.log(triplist_map);
