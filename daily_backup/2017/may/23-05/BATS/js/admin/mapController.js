		...
		$http({
			...
			headers : {
				...
				if(data[0].values.length>0){				
					...
					var latlng = new google.maps.LatLng(data[0].values[0].lat, data[0].values[0].long);
					var html ="";
					...
					geocoder.geocode({latLng: latlng}, function(responses){     
						...
						if(html.length==0){
						    html=responses[0].formatted_address;
						    contentString  = '<b><label>Device ID:</label> '+devIDval+'</b><br><br><b><label>Vehicle No:</label> '+$scope.vehnoSpeedOmeter+'</b><br><br><b><label>Vehicle Model:</label> '+$scope.vehModel+'</b><br><br>'+html+'<br><br><button class="btn btn-primary btn-sm" id="infoClick" data-deviceID="'+devIDval+'">show detail</button>';	
						}		        	   		                    
					    } 
					    else 
					    {       
